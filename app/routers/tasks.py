...
@router.post(path="/", response_model=TaskRead, status_code=201)
def create_task(task: TaskCreate, db: Session = Depends(get_db)) -> Task:
    return crud.create_task(db, task)


@router.get(path="/{task_id}", response_model=TaskRead)
...
@router.get(path="/", response_model=list[TaskRead])
def read_all_tasks(db: Session = Depends(get_db)) -> list[Task]:
    return crud.read_all_tasks(db)


@router.patch(path="/{task_id}", response_model=TaskRead)
def update_task(
...
@router.delete(path="/{task_id}", status_code=204)
